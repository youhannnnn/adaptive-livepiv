...
def piv(settings):
    ...
    def func(args):
        ...
        if settings.dynamic_masking_method in ("edges", "intensity"):
        ...
        if settings.show_plot or settings.save_plot:
            settings.scale_plot = max(abs(np.max(u)), abs(np.min(u)), abs(np.min(v)), abs(np.min(v))) * 19.136
            ...
            fig, _ = display_vector_field(
                ...
                scaling_factor=settings.scaling_factor,
                window_size=settings.windowsizes[settings.num_iterations-1],
